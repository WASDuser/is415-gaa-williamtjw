...
```{r}
pacman:::p_load(sf,tidyverse,janitor) # more to be added
```
...
For a more compact view, I will use `pivot_longer()` to transpose the table and `filter()` to rid off the columns with no missing data
